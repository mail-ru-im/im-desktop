...
#include "macos/mac_certs_cache.h"
...
{
template <typename T>
class RefDeleter : public boost::noncopyable
{
public:
...
core::mac_certs_cache& get_certs_cache()
{
    static core::mac_certs_cache certs_cache;
    return certs_cache;
}
...
namespace core
{
    CURLcode ssl_ctx_callback_mac(CURL *curl, void *ssl_ctx, void *userptr)
    {
        ...
        get_certs_cache().for_each([store](auto x509_cert)
        {
                ...
                for (CFIndex i = 0; i < size; ++i)
                {
                    ...
                    if (isCaCertificateTrusted(cfCert, dom))
                    {
                        if (derData != NULL)
                        {
                            ...
                            if (x509_cert)
                            {
                                auto not_after = X509_getm_notAfter(x509_cert);
                                if (X509_cmp_current_time(not_after) == 1)
                                    result.push_back(x509_cert);
                            }
                        }
                    }
                }
