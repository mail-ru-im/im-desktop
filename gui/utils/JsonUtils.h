...
namespace JsonUtils
{
    ...
    {
        if (const auto iter = _node.FindMember(_name); iter != _node.MemberEnd() && iter->value.IsInt64())
        {
    ...
    template <class U>
    inline bool unserialize_value(const rapidjson::Value& _node, const U& _name, Out std::chrono::seconds& _out_param)
    {
        if (const auto iter = _node.FindMember(_name); iter != _node.MemberEnd() && iter->value.IsInt64())
        {
            _out_param = std::chrono::seconds(iter->value.GetInt64());
            return true;
        }
